...
import { CancelledSaleData } from "../types/Sale";
...
//Get cancelled sales
export const getCancelledSales = async (outletId: string, date: string): Promise<CancelledSaleData[]> => {
  ...
  if (!res.ok) {
    throw new Error("Failed to fetch cancelled sales");
  }
